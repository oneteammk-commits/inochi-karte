...
import { createCard, isValidPasswordFormat } from './cardApi'

/** 画面の入力内容を、データベースに渡す形に変換する */
...
export async function persistRegistration(form: RegistrationFormState): Promise<string> {
  ...
  // 編集用パスワードのチェック（数字4桁）
  if (!isValidPasswordFormat(form.editPassword)) {
    throw new Error('編集用パスワードは数字4桁で入力してください。')
  }

  // パスワードのハッシュ化はデータベース側で行う（ブラウザには渡さない）
  return createCard(buildRegistrationPayload(form), form.editPassword)
}
